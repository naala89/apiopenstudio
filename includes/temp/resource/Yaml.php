...
class ProcessorYaml extends ProcessorResource
{
  ...
  public function process()
  {
    ...
    switch ($this->request->method) {
      ...
      case 'delete':
        ...
        break;
      default:
        throw new \Datagator\includes\ApiException('unknown method', -1, $this->id);
        break;
    }
  ...
  private function _yamlIn()
  {
    ...
    if (empty($yaml)) {
      throw new \Datagator\includes\ApiException('invalid or no yaml supplied', -1, $this->id, 417);
    }
  ...
  private function _yamlOut()
  {
    ...
    if (empty($method)) {
      throw new \Datagator\includes\ApiException('missing get var method', -1, $this->id, 400);
    }
    $resource = $this->getVar($this->meta->resource);
    if (empty($resource)) {
      throw new \Datagator\includes\ApiException('missing get var resource', -1, $this->id, 400);
    }
    $action = $this->getVar($this->meta->action);
    if (empty($action)) {
      throw new \Datagator\includes\ApiException('missing get var action', -1, $this->id, 400);
    }
