...
use ApiOpenStudio\Core\OpenApi\OpenApiParent303;
use ApiOpenStudio\Core\OpenApi\OpenApiPath303;
...
class OpenApi303Test extends Unit
{
    /**
     * @var UnitTester
     */
    ...
    /**
     * @var OpenApiParent303
     */
    protected OpenApiParent303 $openApiParent;

    /**
     * @var OpenApiPath303
     */
    protected OpenApiPath303 $openApiPath;
    ...
    public function testDefault()
    {
        try {
            ...
            $this->fail('An exception was thrown generating the default OpenApi 3.0.3: ' . $e->getMessage());
        }
        try {
            $decodedJson = $this->openApiParent->export(false);
        } catch (ApiException $e) {
            $this->fail('An exception was thrown exporting the default OpenApi 3.0.3: ' . $e->getMessage());
        }
        try {
            $decodedJson->paths = $this->getPathsOpenApi();
            $json = json_encode($decodedJson, JSON_UNESCAPED_SLASHES);
        } catch (ApiException $e) {
            $this->fail('An exception was thrown exporting the default paths OpenApi 3.0.3: ' . $e->getMessage());
        }
        try {
        ...
        $this->assertEquals('3.0.3', $decodedJson->openapi, 'Invalid openapi version');
