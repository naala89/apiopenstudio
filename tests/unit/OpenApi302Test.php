...
use ApiOpenStudio\Core\OpenApi\OpenApiParent302;
use ApiOpenStudio\Core\OpenApi\OpenApiPath302;
...
class OpenApi302Test extends Unit
{
    /**
     * @var UnitTester
     */
    ...
    /**
     * @var OpenApiParent302
     */
    protected OpenApiParent302 $openApiParent;

    /**
     * @var OpenApiPath302
     */
    protected OpenApiPath302 $openApiPath;
    ...
    {
        $this->settings = new Config(dirname(__DIR__) . '/_data/settings.openapi.302.yml');
        $this->openApiParent = new OpenApiParent302($this->settings);
        $this->openApiPath = new OpenApiPath302();
    ...
    public function testDefault()
    {
        try {
            ...
            $this->fail('An exception was thrown generating the default OpenApi 3.0.2: ' . $e->getMessage());
        }
        try {
            $decodedJson = $this->openApiParent->export(false);
        } catch (ApiException $e) {
            $this->fail('An exception was thrown exporting the default OpenApi 3.0.2: ' . $e->getMessage());
        }
        try {
            $decodedJson->paths = $this->getPathsOpenApi();
            $json = json_encode($decodedJson, JSON_UNESCAPED_SLASHES);
        } catch (ApiException $e) {
            $this->fail('An exception was thrown exporting the default paths OpenApi 3.0.2: ' . $e->getMessage());
        }
        try {
        ...
        $this->assertEquals('3.0.2', $decodedJson->openapi, 'Invalid openapi version');
